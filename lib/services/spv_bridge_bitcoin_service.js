...
var SPVBridgeEndpointMainnet = 'http://localhost:8080/'; // TODO: develop
var SPVBridgeEndpointTestnet = undefined;
...
var SPVBridgeBitcoinService = function(options) {
    ...
    if (typeof options.host === 'undefined' && options.testnet) {
        this.defaultSettings.host = SPVBridgeEndpointTestnet;
    }

    this.settings = _.merge({}, this.defaultSettings, options);
    this.DEFAULT_ENDPOINT_MAINNET = SPVBridgeEndpointMainnet;
    this.DEFAULT_ENDPOINT_TESTNET = SPVBridgeEndpointTestnet;
};
    ...
    self.postEndpoint('addressListUnspent', data).then(function(results) {
        ...
        results.forEach(function(utxo) {
            ...
            batchResults[address].push({
                ...
                'index': utxo['tx_pos'],
                'value': utxo['value'],
                'script_hex': blocktrail.bitcoin.address.toOutputScript(address, blocktrail.bitcoin.networks.bitcoincash),
