...
var BlocktrailBitcoinService = function(options) {
    ...
    var normalizedNetwork = BlocktrailSDK.normalizeNetworkFromOptions(options);
    this.settings.network = normalizedNetwork[0];
    this.settings.testnet = normalizedNetwork[1];
    this.settings.regtest = normalizedNetwork[2];
    // apiNetwork we allow to be customized for debugging purposes
    this.settings.apiNetwork = this.settings.apiNetwork || normalizedNetwork[3];
