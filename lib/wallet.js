 ...
 * @constructor
 */
var Wallet = function (sdk, identifier, primaryMnemonic, backupPublicKey, blocktrailPublicKeys, keyIndex, testnet, checksum, upgradeToKeyIndex) {
    var self = this;
...
Wallet.prototype.unlock = function (options, cb) {
    ...
    try {
        ...
        var primaryMnemonic = typeof options.primaryMnemonic !== "undefined" ? options.primaryMnemonic : self.primaryMnemonic;
        ...
        self.sdk.resolvePrimaryPrivateKey(primaryPrivateKey, primaryMnemonic, passphrase).then(function(primaryPrivateKey) {
    ...
    return deferred.promise.then(function(wallet) {
        // create a checksum of our private key which we'll later use to verify we used the right password
        var checksum = wallet.primaryPrivateKey.getAddress().toBase58Check();

        // check if we've used the right passphrase
        if (checksum !== self.checksum) {
            throw new Error("Generated checksum [" + checksum + "] does not match [" + wallet.checksum + "], most likely due to incorrect password");
        }

        // if the response suggests we should upgrade to a different blocktrail cosigning key then we should
        if (wallet.upgradeToKeyIndex) {
            wallet.upgradeKeyIndex(wallet.upgradeToKeyIndex);
        }
