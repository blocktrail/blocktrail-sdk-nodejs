...
// Libraries for rootstock, ethereum key management.
var ethKey = require('ethereumjs-wallet/hdkey');
...
var Wallet = function(
    ...
    // If this is not rootstock, make sure back up and blocktrail keys are present.
    if (self.sdk.chain !== "RSK") {
...
Wallet.RSK_STANDARD_TRANSACTION_GAS_LIMIT = 21000;
Wallet.RSK_MINIMUM_GAS_PRICE = 0;
...
/**
 * generate a new derived private key and return the rootstock address for it
 *
 * @param [chainIdx] int
 * @param [cb]  function        callback(err, address)
 * @returns {q.Promise}
 */
Wallet.prototype.getRskAddress = function(chainIdx, cb) {
    var self = this;
    ...
    if (chainIdx !== parseInt(chainIdx, 10)) {
        ...
        if (typeof chainIdx === "undefined" || chainIdx === null) {
            chainIdx = Wallet.CHAIN_RSK_DEFAULT;
        } else {
    ...
    // Get Rootstock address from server.
    deferred.resolve(self.sdk.getNewDerivation(self.identifier, "M/" + self.keyIndex + "'/" + chainIdx)
        .then(function(newDerivation) {
            var path = newDerivation.path;
            var addressFromServer = newDerivation.address;

            // Verify Rootstock address client-side.
            var verifyAddress = self.getRskAddressByPath(path);

            // Check that server side address is equal to our client side address.
            if (verifyAddress !== addressFromServer) {
                throw new blocktrail.WalletAddressError("Failed to verify address [" + addressFromServer + "] !== [" + verifyAddress + "]");
            }
...
/**
 * get rootstock address for specified path
 *
 * @param path
 * @returns string
 */
Wallet.prototype.getRskAddressByPath = function(path) {
    var self = this;

    // Get the derived primary public key.
    var derivedPrimaryPublicKey = self.getPrimaryPublicKey(path);
    
    // Encode the derived key to base58.
    var encodedPublicKey = derivedPrimaryPublicKey.toBase58();

    // Instantiate an HD ethereum key using the extended public key.
    var derivedEthereumKey = ethKey.fromExtendedKey(encodedPublicKey);

    // Use the public key to encode an ethereum address (without address checksum).
    var address = derivedEthereumKey.getWallet().getAddressString();
    return address;
};
...
/**
 *  Performs steps to create a valid transaction for rootstock networks.
 *
 * @param pay      {value, to}  value of transaction to create and recipient address
 * @param feeStrategy      {number}
 * @param twoFactorToken
 * @param options
 * @param cb
 * @returns {promise}
 */
Wallet.prototype.rskPay = function(pay, feeStrategy, twoFactorToken, options, cb) {
    /* jshint -W071 */
    var self = this;
    ...
    feeStrategy = feeStrategy || Wallet.RSK_MINIMUM_GAS_PRICE;
    options = options || {};
    ...
    q.nextTick(function() {
        self.buildRskTransaction(pay, feeStrategy, options).then(
            function(r) { return r; },
            function(e) { deferred.reject(e); },
        ).spread(
            function(transaction) {
                return self.sendRskTransaction(transaction.tx, twoFactorToken, options)
                    .then(function(result) {
                        if (!result || !result['complete'] || result['complete'] === 'false') {
                            deferred.reject(new Error("Failed to send transaction."));
                        } else {
...
/**
 * Builds a standard rootstock transaction and serializes the valid transaction.
 *  Standard transactions should only pay to other externally owned accounts.
 *  Standard transactions cannot interact with methods defined by contract accounts.
 *
 * @todo: Implement dynamic fee policy.
 * @param pay      {to, value}
 * @param feeStrategy      {number}
 * @param options
 * @param cb
 * @returns {promise}
 */
Wallet.prototype.buildRskTransaction = function(pay,feeStrategy, options, cb) {
    /* jshint -W071 */
    var self = this;
    ...
    // If no feeStrategy is provided, use the minimum gas price.
    feeStrategy = feeStrategy || Wallet.RSK_MINIMUM_GAS_PRICE;
    options = options || {};
    ...
    var path = "M/" + self.keyIndex + "'/" + Wallet.CHAIN_RSK_DEFAULT + "/" + 0;
    var accountAddress = self.getRskAddressByPath(path);

    q.nextTick(function() {
        deferred.resolve(
            // Get details about the account we plan to spend from.
            self.getRskAccount(accountAddress).spread(function(result){
                var confirmed_balance = result.confirmed_balance;
                var address = result.address;
                var path = result.path;
                var nonce = result.nonce;

                var txb, transaction,tx, txId;
                var deferred = q.defer();
                async.waterfall([
                    /**
                     * init transaction builder
                     ...
                     */
                    function(cb) {
                        // ChainId 30 is Rootstock Mainnet
                        // ChainId 31 is Rootstock Testnet

                        if (self.sdk.rskTestnet) {
                            txb = new ethTransaction(null,31);
                        } else {
                            txb = new ethTransaction(null,30);
                        }
                    ...
                    function(cb) {
                        ...
                        txb.nonce = nonce;
                        txb.gasPrice = feeStrategy;
                        txb.gasLimit = Wallet.RSK_STANDARD_TRANSACTION_GAS_LIMIT;
                        txb.value = web3.utils.toHex(pay.value);
                        txb.to = pay.to;
                        
                        // Check if transaction value < confirmed balance of account.
                        {
                            var gasLimit = web3.utils.toBN(Wallet.RSK_STANDARD_TRANSACTION_GAS_LIMIT);
                            var gasPrice = web3.utils.toBN((feeStrategy ? feeStrategy : 1));
                            var maximumGasFee = gasLimit.mul(gasPrice)
                            var totalTxValue = web3.utils.toBN(pay.value).add(maximumGasFee);
                            var balance = web3.utils.toBN(confirmed_balance);

                            if (totalTxValue.gte(balance)) {
                                throw new Error("Total transaction value is more than account value.");
                            }
                        }
                    ...
                    function(cb) {
                        ...
                        if (self.primaryPrivateKey) {
                            privKey = Wallet.deriveByPath(self.primaryPrivateKey, path, "m");
                        } else {
                            throw new Error("No master privateKey present");
                        }

                        txb.sign(ethUtil.toBuffer("0x" + privKey.keyPair.d.toBuffer().toString('hex')));

                        // Validate transaction before serialization.
                        if (!txb.validate()) {
                            throw new Error("Unable to create a valid transaction.");
                        }

                        var serializedTx = txb.serialize();
                        tx = serializedTx.toString('hex');
                        txId = new ethTransaction(tx).hash().toString('hex');
                        ...
                        // Check if the provided account matches the finished valid transaction.
                        if (address !== "0x" + txb.from.toString('hex')) {
                            throw new Error("Problem signing the transaction.");
                        }
...
/**
 *  Prepares to broadcast serialized rootstock transaction.
 *
 * @param tx
 * @param twoFactorToken
 * @param options
 ...
 * @returns {promise}
 */
Wallet.prototype.sendRskTransaction = function(tx, twoFactorToken, options, cb) {
    var self = this;
    ...
    self.sdk.sendRskTransaction(self.identifier, tx, twoFactorToken, options)
        .then(
            function(result) {
                deferred.resolve(result);
            },
...
Wallet.prototype.getRskAccount = function(address, options, cb) {
    var self = this;
...
Wallet.prototype.decodeRskAddress = function(address) {
    return (
        ethUtil.isValidAddress(address) ?
        {address: address, decoded: address, type: "rsk"} :
        new blocktrail.InvalidAddressError("Invalid Rootstock Address."));
};
