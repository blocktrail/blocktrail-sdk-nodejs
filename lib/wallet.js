...
Wallet.prototype.getRedeemScriptByPath = function(path) {
    ...
    // create multisig
    return bitcoin.script.multisigOutput(2, pubKeys);
};
...
Wallet.prototype.deleteWallet = function(force, cb) {
    ...
    var checksum = self.primaryPrivateKey.getAddress();
    var signature = bitcoin.message.sign(self.primaryPrivateKey.keyPair, checksum, self.network).toString('base64');
    ...
    q.nextTick(function() {
        ...
        Object.keys(pay).forEach(function(address) {
            ...
            if (address === Wallet.OP_RETURN) {
                ...
                send.push({scriptPubKey: bitcoin.script.nullDataOutput(datachunk).toString('hex'), value: 0});
                return;
            }
