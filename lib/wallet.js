...
Wallet.estimateIncompleteTxFee = function(tx) {
    ...
    tx.ins.forEach(function(txin) {
        ...
        // @TODO: get the N of M from the above code
        var multisig = [2, 3]; // we only do 2 of 3 multisig atm
    ...
    self.coinSelection(
        ...
        function(err, utxos, fee, change) {
            ...
            async.waterfall([
                ...
                function(cb) {
                    self.sendTransaction(tx.toHex(), utxos.map(function(utxo) { return utxo['path']; }), cb);
                }
            ], function(err, result) {
                if (err) {
                    return deferred.reject(err);
...
Wallet.prototype.sendTransaction = function(txHex, paths, cb) {
    var self = this;

    return self.sdk.sendTransaction(self.identifier, txHex, paths, cb);
};
