 ...
 * @returns {number}
 */
Wallet.estimateInputsSize = function (utxos, withWitness) {
    var inputSize = 0;
    var witnessSize = 0;
...
Wallet.estimateVsizeFee = function (tx, utxos, feePerKb) {
    var vsize = this.estimateTxVsize(tx, utxos);
 ...
 * @returns {Number}
 */
Wallet.estimateTxVsize = function (tx, utxos) {
    return parseInt(Math.ceil(this.estimateTxWeight(tx, utxos) / 4), 10);
};
 ...
 * @returns {*}
 */
Wallet.estimateTxWeight = function (tx, utxos) {
    var outSize = this.calculateOutputsSize(tx);
 ...
 * @returns {number}
 */
Wallet.calculateOutputsSize = function (tx) {
    var outputsSize = 0;
    tx.outs.map(function (out) {
