...
Wallet.prototype.pay = function (pay, changeAddress, allowZeroConf, randomizeChangeIdx, cb) {
    ...
    self.buildTransaction(pay, changeAddress, allowZeroConf, randomizeChangeIdx).spread(
        function(tx, utxos) {
            return self.sendTransaction(tx.toHex(), utxos.map(function (utxo) { return utxo['path']; }), true)
                ...
                function(result) {
                    if (!result || !result['complete'] || result['complete'] === 'false') {
                        return deferred.reject(new Error("Failed to completely sign transaction"));
                    } else {
                        return result['txid'];
                    }
                }
                    ...
                    function (cb) {
                        if (change > 0) {
                            if (randomizeChangeIdx) {
                                console.log(outputs, _.random(0, outputs.length));
                                outputs.splice(_.random(0, outputs.length), 0, {address: changeAddress, value: change});
                                console.log(outputs);
                            } else {
