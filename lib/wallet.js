...
Wallet.prototype.buildTransaction = function(pay, changeAddress, allowZeroConf, randomizeChangeIdx, feeStrategy, options, cb) {
    ...
    q.nextTick(function() {
        ...
        deferred.resolve(
             ...
             * @returns {*}
             */
                .spread(function(utxos, fee, change, result) {
                    var tx, txb, outputs = [];

                    var bitcoinCashOpreturn = result.bitcoincash_opreturn;

                    if (bitcoinCashOpreturn && hasOpreturn) {
                        throw new Error("Can't add replay protection when transaction also contains opreturn");
                    }
                    ...
                    async.waterfall([
                        ...
                        function(cb) {
                            ...
                            if (bitcoinCashOpreturn) {
                                var datachunk = new Buffer(bitcoinCashOpreturn, 'utf-8');
                                txb.addOutput(bitcoin.scripts.nullDataOutput(datachunk), 0);
                            }
