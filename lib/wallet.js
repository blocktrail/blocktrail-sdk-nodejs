...
var ethereumKey = require('ethereumjs-wallet/hdkey')
...
Wallet.prototype.getRskAddressByPath = function(path) {
    ...
    var derivedEthereumKey = ethereumKey.fromExtendedKey(encodedPublicKey);
...
Wallet.prototype.rskPay = function(pay, feeStrategy, twoFactorToken, options, cb) {
    ...
    q.nextTick(function() {
        self.buildRskTransaction(pay, feeStrategy, options).then(
            ...
            .spread(
            function(signedTx,txid) {         
                return self.sendRskTransaction(signedTx,txid, checkFee, twoFactorToken, options.prioboost, options)
                    .then(function(result) {
                        if (!result || !result['complete'] || result['complete'] === 'false') {
    ...
    q.nextTick(function() {
        deferred.resolve(
            self.getRskAccount(accountAddress).spread(function(confirmed_balance,address,path,nonce){
                var txb, serializedTx, transaction, txId;
                var deferred = q.defer();
                async.waterfall([
                    /**
                     * init transaction builder
                    ...
                    function(cb) {
                        ...
                        //@TODO - implement safety check for nonce
                        txb.nonce = nonce;
                        txb.gasPrice = Wallet.RSK_MINIMUM_GAS_PRICE;
                    ...
                    function(cb) {
                        ...
                        // @TODO - validate transaction before serialization
                        // txb.validate();

                        transaction = txb.serialize();
                        serializedTx = transaction.toString('hex');
                        txId = new ethTransaction(serializedTx).hash().toString('hex');
                    ...
                    deferred.resolve([serializedTx,txId]);
                });
...
Wallet.prototype.sendRskTransaction = function(tx,txid, checkFee, twoFactorToken, prioboost, options, cb) {
    var self = this;
    ...
    self.sdk.sendRskTransaction(self.identifier, tx, txid, checkFee, twoFactorToken, prioboost, options)
        .then(
            function(result) {
                deferred.resolve(result);
            },
