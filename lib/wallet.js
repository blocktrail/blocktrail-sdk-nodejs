    ...
    deferred.resolve(self.sdk.getNewDerivation(self.identifier, "M/" + self.keyIndex + "'/" + chainIdx)
        .then(function(newDerivation) {
            ...
            if (verifyAddress !== addressFromServer) {
                deferred.reject(new Error("Failed to verify address [" + addressFromServer + "] !== [" + verifyAddress + "]"));
            }
...
Wallet.prototype.buildRskTransaction = function(pay,feeStrategy, options, cb) {
    ...
    q.nextTick(function() {
        deferred.resolve(
            ...
            self.getRskAccount(accountAddress).spread(function(confirmed_balance,address,path,nonce){
                ...
                async.waterfall([
                    ...
                    function(cb) {
                        ...
                        // Check if transaction value < confirmed balance of account.
                        {
                            ...
                            var totalTxValue = web3.utils.toBN(value).add(maximumGasFee);
                            var balance = web3.utils.toBN(confirmed_balance);
                            if (totalTxValue.gt(balance)) {
