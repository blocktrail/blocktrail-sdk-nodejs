var _ = require('lodash');
var pdf = require('html-pdf');
var qrcode = require('qrcode-js/lib/qrcode.js');

var QrCode = function(text, size, typeNumber, errorCorrectLevel) {
    var qr = qrcode(typeNumber || 4, errorCorrectLevel || 'L');
    qr.addData(text);
    qr.make();
    var base64 = qr.createImgBase64(size);
    var dataURL = 'data:image/gif;base64,' + base64;

    return dataURL;
};

/**
 * @param primaryMnemonic       string          primary key mnemonic
 * @param backupMnemonic        string          backup key mnemonic
 * @param blocktrailPublicKeys  array           list of blocktrail pubKeys indexed by keyIndex
 * @constructor
 */
var BackupGenerator = function (primaryMnemonic, backupMnemonic, blocktrailPublicKeys) {
    var self = this;

    self.primaryMnemonic = primaryMnemonic;
    self.backupMnemonic = backupMnemonic;
    self.blocktrailPublicKeys = [];

    _.each(blocktrailPublicKeys, function(pubKey, keyIndex) {
        self.blocktrailPublicKeys.push({
            keyIndex: keyIndex,
            pubKey:   pubKey[0],
            path:     pubKey[1],
            qr:       QrCode(pubKey[0], 3, 10)
        });
    });
};

/**
 * create an HTML version of the backup document
 *
 */
BackupGenerator.prototype.generateHTML = function() {
    var self = this;
    var pubKeysHtml = "";
    _.each(self.blocktrailPublicKeys, function(pubKey, index) {
        pubKeysHtml += "<figure><img src='" + pubKey.qr + "' /><figcaption>";
        pubKeysHtml += "<span>KeyIndex: " + pubKey.keyIndex + " </span> ";
        pubKeysHtml += "<span>Path: " + pubKey.path + "</span>";
        pubKeysHtml += "</figcaption></figure>";
    });
    totalPubKeys = self.blocktrailPublicKeys.length;

    return "<html>"
        + "<head>"
        + "<title>Wallet Backup Info</title>"

        + "<link href='http://fonts.googleapis.com/css?family=Open+Sans:400,300' rel='stylesheet' type='text/css'>"

        + "<style>"
        + "@page {margin: 3.5em 2.5em 3.5em 2.5em; }"
        + "html { font-size: 100%; }"
        + "body, h1, h2, h3 { font-family: 'Open Sans', Helvetica, sans-serif; font-weight: 100; }"
        + "h1, h2, h3 { font-weight: 100; }"
        + "h3 { color: #333; }"
        + "p { margin: 0.5em 0; line-height: 1em; }"
        + "figure { display: inline-block; margin: 1em;}"
        + "figcaption { font-size: 0.8em; text-align: center;}"
        + "header { position: fixed; top: -2.5em; right: 0; width: 8em; height: 5em; opacity: 0.7; }"
        + ".logo-blocktrail-square { display: block; }"
        + ".logo-blocktrail-square img{  width:100%; height:auto; }"
        + "footer { position: fixed; left: 0; bottom: -5em; right: 0; height: 4em; text-align:right; font-size: 0.8em; color: #666; }"
        + "footer .page:after { content: counter(page); }"
        + ".intro h1 { margin:0; padding: 0; }"
        + ".backup-info { padding-bottom: 1em; margin-bottom: 1em; }"
        + ".backup-info small { display: block; color: #666; font-size: 0.75em; }"
        + ".backup-info figcaption span:first-child { margin-right: 1em; }"
        + "section h2 { padding-bottom: 0.2em; margin-bottom: 0.2em; border-bottom: 1px solid #CCC; }"
        + "</style>"
        + "</head>"

        + "<body>"
        + "<footer>"
        + "<p class='page'>Page </p>"
        + "</footer>"
        + "<header>"
        + "<a class='logo-blocktrail-square' href='https://www.blocktrail.com/'>"
        + "<img src='data:image/png;base64,'  alt='blocktrail' />"
        + "</a>"
        + "</header>"

        + "<section class='intro'>"
        + "<h1>Wallet Recovery Data Sheet</h1>"
        + "<p>This document holds the information and instructions required for you to recover your Blocktrail wallet should anything happen. <br>"
        + "Print it out and keep it in a safe location; if you lose these details you will never be able to recover your wallet.</p>"
        + "</section>"

        + "<section class='backup-info'>"
        + "<h2>Backup Info</h2>"
        + "<div class='primary-mnemonic'><h3>Primary Mnemonic</h3><code>" + self.primaryMnemonic + "</code></div>"

        + "<div class='backup-mnemonic'><h3>Backup Mnemonic</h3><code>" + self.backupMnemonic + "</code></div>"

        + "<div class='blocktrail-pubkeys'><h3>Blocktrail Public Keys <small>" + totalPubKeys + " in total</small></h3>"
        + pubKeysHtml
        + "</div>"
        + "</section>"

        + "<section class='backup-instructions'>"
        + "<div><h2>Wallet Recovery Instructions</h2><p>For instructions on how to recover your wallet, see the 'wallet_recovery_example.php' script in the examples folder of the Blocktrail SDK.</p></div>"
        + "</section>"
        + "</body>"
        + "</html>";
};


/**
 * create a PDF version of the backup document
 *
 */
BackupGenerator.prototype.generatePDF = function() {
    var self = this;
    var html = self.generateHTML();
};


module.exports = BackupGenerator;
