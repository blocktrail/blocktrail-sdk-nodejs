var async = require('async');
var _ = require('lodash');
var htmlToFile = require('html-pdf');
var fs = require('fs');
var QRCode = require('./qrCode');

var isNodeJS = !process.browser;

/**
 * @param primaryMnemonic       string          primary key mnemonic
 * @param backupMnemonic        string          backup key mnemonic
 * @param blocktrailPublicKeys  array           list of blocktrail pubKeys indexed by keyIndex
 * @constructor
 */
var BackupGenerator = function (primaryMnemonic, backupMnemonic, blocktrailPublicKeys) {
    var self = this;

    self.primaryMnemonic = primaryMnemonic;
    self.backupMnemonic = backupMnemonic;
    self.blocktrailPublicKeys = [];

    _.each(blocktrailPublicKeys, function (pubKey, keyIndex) {
        self.blocktrailPublicKeys.push({
            keyIndex: keyIndex,
            pubKey:   pubKey,
            path:     "M/" + keyIndex + "'"
        });
    });
};

/**
 * create an HTML version of the backup document
 *
 */
BackupGenerator.prototype.generateHTML = function (cb) {
    var self = this;
    var data = {
        primaryMnemonic: self.primaryMnemonic,
        backupMnemonic: self.backupMnemonic,
        totalPubKeys: self.blocktrailPublicKeys.length,
        pubKeysHtml: ""
    };

    async.forEach(Object.keys(self.blocktrailPublicKeys), function(keyIndex) {
        var pubKey = self.blocktrailPublicKeys[keyIndex];

        QRCode.toDataURL(pubKey.pubKey.toBase58(), {
            errorCorrectLevel: 'medium'
        }, function(err, dataURI) {
            pubKey.qr = dataURI;
        });
    }, function(err) {
        if (err) {
            return cb(err);
        }

        _.each(self.blocktrailPublicKeys, function (pubKey) {
            data.pubKeysHtml += "<figure><img src='" + pubKey.qr + "' /><figcaption>";
            data.pubKeysHtml += "<span>KeyIndex: " + pubKey.keyIndex + " </span> ";
            data.pubKeysHtml += "<span>Path: " + pubKey.path + "</span>";
            data.pubKeysHtml += "</figcaption></figure>";
        });

        var compiledHtml =  _.template(fs.readFileSync(__dirname + "/resources/backup_info_template.html", 'utf8'));
        cb(null, compiledHtml(data));
    });
};

var pdfer = function(options, generate) {
    this.doc = new jsPDF('portrait', 'pt', 'letter');

    this.docWidth = 612;
    this.margin = typeof options.margin !== "undefined" ? options.margin : 30;
    this.bodyWidth = 612 - (this.margin * 2);
    this.yPos = 0;

    this.fontSize = [
        14
    ];
    this.lineMargin = [
        [2, 2]
    ];
    this.textColor = [
        [0, 0, 0]
    ];
    this.font = [
        'helvetica'
    ];

    generate(this);
};

pdfer.prototype.setFont = function(font, cb) {
    this.font.push(font);
    this.doc.setFont(this.font[this.font.length -1 ]);

    if (cb) {
        cb();
    }

    this.font.pop();
    this.doc.setFont(this.font[this.font.length -1 ]);
};

pdfer.prototype.setFontSize = function(size, lineMargin, cb) {
    this.fontSize.push(size);
    this.lineMargin.push(lineMargin);
    this.doc.setFontSize(this.fontSize[this.fontSize.length -1 ]);

    if (cb) {
        cb();
    }

    this.fontSize.pop();
    this.lineMargin.pop();
    this.doc.setFontSize(this.fontSize[this.fontSize.length -1 ]);
};

pdfer.prototype.setTextColor = function(color, cb) {
    this.textColor.push(color);
    this.doc.setTextColor.apply(this.doc, this.textColor[this.textColor.length -1 ]);

    if (cb) {
        cb();
    }

    this.textColor.pop();
    this.doc.setTextColor.apply(this.doc, this.textColor[this.textColor.length -1 ]);
};

pdfer.prototype.TEXT = function(text, leftOffset, autoY, cb) {
    var self = this;

    if (typeof leftOffset === "function") {
        cb = leftOffset;
        leftOffset = null;
    } else if (typeof autoY === "function") {
        cb = autoY;
        autoY = null;
    }

    if (typeof autoY === "undefined" || autoY === null) {
        autoY = true;
    }

    var lines = self.doc.splitTextToSize(text, self.docWidth * 2);

    if (autoY) {
        var currentSize = self.fontSize[self.fontSize.length -1 ];
        var currentLineMargin = self.lineMargin[self.lineMargin.length -1 ];

        lines.forEach(function(line) {
            self.YAXIS(currentLineMargin[0]);
            self.YAXIS(currentSize);
            self.doc.text(line, self.margin + (leftOffset || 0), self.yPos);
            self.YAXIS(currentLineMargin[1]);
        });
    } else {
        self.doc.text(lines, self.margin + (leftOffset || 0), self.yPos);
    }

    if (cb) {
        cb();
    }
};

pdfer.prototype.HR = function(xOffset, yOffset) {
    var x1 = (xOffset || 0);
    var x2 = this.docWidth - this.margin - this.margin - (xOffset || 0);

    var y = (yOffset || 0);

    this.LINE(x1, y, x2, y);
};

pdfer.prototype.LINE = function(x1, y1, x2, y2) {
    this.doc.line(x1 + this.margin, y1 + this.yPos, x2 + this.margin, y2 + this.yPos);
};

pdfer.prototype.IMAGE = function(img, format, width, height, x) {
    x = (x || 0) + this.margin;
    var y = this.yPos;
    var w = width;
    var h = height;

    this.doc.addImage(img, format, x, y, w, h, undefined, 'none');
    this.YAXIS(h);
};

pdfer.prototype.YAXIS = function(y) {
    this.yPos += y || 0;
};

pdfer.prototype.NEXT_PAGE = function() {
    this.doc.addPage();
    this.yPos = 0;
};

pdfer.prototype.FONT_SIZE_HEADER = function(cb) { this.setFontSize(24, [12, 8], cb); };
pdfer.prototype.FONT_SIZE_SUBHEADER = function(cb) { this.setFontSize(18, [8, 5], cb); };
pdfer.prototype.FONT_SIZE_NORMAL = function(cb) { this.setFontSize(13, [2, 2], cb); };
pdfer.prototype.FONT_SIZE_SMALL = function(cb) { this.setFontSize(10, [2, 2], cb); };

pdfer.prototype.TEXT_COLOR_BLACK = function(cb) { this.setTextColor([0, 0, 0], cb); };
pdfer.prototype.TEXT_COLOR_GREY = function(cb) { this.setTextColor([51, 51, 51], cb); };
pdfer.prototype.TEXT_COLOR_RED = function(cb) { this.setTextColor([255, 0, 0], cb); };

/**
 * create a PDF version of the backup document
 */
BackupGenerator.prototype.generatePDF = function (callback) {
    /* jshint -W101 */
    var self = this;

    var pdf = new pdfer({
        leftMargin: 30
    }, function(pdf) {
        pdf.setFont('helvetica'); // default font

        pdf.YAXIS(30); // top margin

        pdf.IMAGE(
            'data:image/jpeg;base64,',
            'jpeg',
            183,
            25
        );

        pdf.FONT_SIZE_HEADER(function() {
            pdf.TEXT("Wallet Recovery Data Sheet");
        });

        pdf.TEXT(
            "This document holds the information and instructions required for you to recover your Blocktrail wallet should anything happen. \n" +
             "Print it out and keep it in a safe location; if you lose these details you will never be able to recover your wallet."
        );

        pdf.FONT_SIZE_HEADER(function() {
            pdf.TEXT("Backup Info");
            pdf.HR(0, 0);
        })

        pdf.FONT_SIZE_SUBHEADER(function() {
            pdf.TEXT_COLOR_GREY(function() {
                pdf.TEXT("Primary Mnemonic");
            });
            pdf.YAXIS(5);
            pdf.FONT_SIZE_NORMAL(function() {
                pdf.TEXT(self.primaryMnemonic);
            });
        });

        pdf.FONT_SIZE_SUBHEADER(function() {
            pdf.TEXT_COLOR_GREY(function() {
                pdf.TEXT("Backup Mnemonic");
            });
            pdf.YAXIS(5);
            pdf.FONT_SIZE_NORMAL(function() {
                pdf.TEXT(self.backupMnemonic);
            });
        });

        if (self.blocktrailPublicKeys.length > 3) {
            pdf.NEXT_PAGE();
            pdf.YAXIS(30); // top margin

            pdf.IMAGE(
                'data:image/jpeg;base64,',
                'jpeg',
                183,
                25
            );

            pdf.YAXIS(10); // need a little extra margin
        }

        pdf.FONT_SIZE_SUBHEADER(function() {
            pdf.TEXT_COLOR_GREY(function() {
                pdf.TEXT("BlockTrail Public Keys");
            });
            pdf.FONT_SIZE_NORMAL(function() {
                pdf.TEXT(self.blocktrailPublicKeys.length + " in total");
            });

        });
        pdf.YAXIS(20);

        async.forEach(Object.keys(self.blocktrailPublicKeys), function(keyIndex, cb) {
            var pubKey = self.blocktrailPublicKeys[keyIndex];

            QRCode.toDataURL(pubKey.pubKey.toBase58(), {
                errorCorrectLevel: 'medium'
            }, function(err, dataURI) {
                pubKey.qr = dataURI;
                cb(err);
            });
        }, function(err) {
            if (err) {
                return cb(err);
            }

            var qrSize = 180;
            var qrSubtitleheight = 20;

            Object.keys(self.blocktrailPublicKeys).forEach(function(keyIndex, i) {
                var pubKey = self.blocktrailPublicKeys[i];

                var x = i % 3;

                // move the yPos back up
                if (i > 0 && x !== 0) {
                    pdf.YAXIS(-qrSize);
                    pdf.YAXIS(-3);
                }

                pdf.IMAGE(pubKey.qr, 'jpeg', qrSize, qrSize, x * qrSize);
                pdf.YAXIS(3);
                pdf.FONT_SIZE_SMALL(function() {
                    pdf.TEXT("KeyIndex: " + pubKey.keyIndex + " Path: " + pubKey.path, (x * qrSize) + 20, false);
                });
            });
            pdf.YAXIS(qrSubtitleheight);

            pdf.NEXT_PAGE();
            pdf.YAXIS(30); // top margin

            pdf.IMAGE(
                'data:image/jpeg;base64,',
                'jpeg',
                183,
                25
            );

            pdf.FONT_SIZE_HEADER(function() {
                pdf.TEXT("Wallet Recovery Instructions");
                pdf.HR(0, 0);
            });

            pdf.TEXT(
                "For instructions on how to recover your wallet, \n" +
                "see the 'wallet_recovery_example.php' script in the examples folder of the Blocktrail SDK."
            );
        });

        callback(null, pdf.doc);
    });
};

/**
 * create an image version of the backup document
 *
 */
BackupGenerator.prototype.generateImage = function (filename, callback) {
    var self = this;

    if (!isNodeJS) {
        throw new Error("Can only use .generateHTML and .generatePDF in browser context");
    }

    if (typeof filename === 'function') {
        callback = filename;
        filename = null;
    }

    var options = {
        // File options
        type: "png",                // allowed file types: png, jpeg, pdf
        quality: "75"               // only used for types png & jpeg
    };

    var html = self.generateHTML();

    if (typeof filename === 'undefined' || filename === null || filename === "") {
        htmlToFile.create(html, options).toBuffer(function (err, result) {
            if (typeof callback === "function") {
                callback(err, result);
            }
        });
    } else {
        htmlToFile.create(html, options).toFile(filename, function (err, result) {
            if (typeof callback === "function") {
                callback(err, result);
            }
        });
    }
};

module.exports = BackupGenerator;
