...
SizeEstimation.estimateStackSignatureSize = function (stackSizes, isWitness, rs, ws) {
    ...
    var self = this;
    var scriptSigSize = scriptSigSizes.reduce(function (acc, elementLen) {
        return acc + self.getLengthForScriptPush(elementLen) + elementLen
    });
    scriptSigSize += self.getLengthForVarInt(scriptSigSize);

    var witnessSize = 0;
    if (witnessSizes.length > 0) {
        witnessSize = witnessSizes.reduce(function (acc, elementLen) {
            return acc + self.getLengthForVarInt(elementLen) + elementLen
        });
