...
var determineDataStorageV2_3 = function(options) {
    ...
    if (typeof options.storeDataOnServer === "undefined") {
        options.storeDataOnServer = !options.primaryPrivateKey;
    }
};

var produceEncryptedDataV2 = function(deferred, options) {
    if (options.storeDataOnServer) {
        if (!options.secret) {
            ...
            deferred.notify(APIClient.CREATE_WALLET_PROGRESS_ENCRYPT_SECRET);
        ...
        deferred.notify(APIClient.CREATE_WALLET_PROGRESS_ENCRYPT_PRIMARY);
        ...
        deferred.notify(APIClient.CREATE_WALLET_PROGRESS_ENCRYPT_RECOVERY);
...
var produceEncryptedDataV3 = function(deferred, options) {
    if (options.storeDataOnServer) {
        if (!options.secret) {
            if (!options.passphrase) {
                throw new blocktrail.WalletCreateError("Can't encrypt data without a passphrase");
            ...
            // -> now a buffer
            options.encryptedSecret = Encryption.encrypt(options.secret, new Buffer(options.passphrase));
        } else {
        ...
        // -> now a buffer
        options.encryptedPrimarySeed = Encryption.encrypt(options.primarySeed, options.secret);

        deferred.notify(APIClient.CREATE_WALLET_PROGRESS_ENCRYPT_RECOVERY);

        options.recoverySecret = randomBytes(Wallet.WALLET_ENTROPY_BITS / 8);
        options.recoveryEncryptedSecret = Encryption.encrypt(options.secret, options.recoverySecret);
    }
};

var doRemainingWalletDataV2_3 = function(network, deferred, options) {
...
APIClient.prototype._createNewWalletV2 = function(options) {
    ...
    q.fcall(function() {
        ...
        produceEncryptedDataV2(deferred, options);
        doRemainingWalletDataV2_3(network, deferred, options);
...
APIClient.prototype._createNewWalletV3 = function(options) {
    ...
    q.fcall(function() {
        ...
        produceEncryptedDataV3(deferred, options);
        doRemainingWalletDataV2_3(network, deferred, options);
                ...
                function(result) {
                    ...
                    return wallet.unlock({
                        ...
                        return [
                            wallet,
                            {
                                encryptedPrimarySeed: options.encryptedPrimarySeed ? Mnemonic.encode(options.encryptedPrimarySeed) : null,
                                backupSeed: options.backupSeed ? bip39.entropyToMnemonic(options.backupSeed) : null,
                                recoveryEncryptedSecret: options.recoveryEncryptedSecret ? Mnemonic.encode(options.recoveryEncryptedSecret) : null,
                                encryptedSecret: options.encryptedSecret ? Mnemonic.encode(options.encryptedSecret) : null,
                                blocktrailPublicKeys: blocktrailPublicKeys
                            }
                        ];
                    });
