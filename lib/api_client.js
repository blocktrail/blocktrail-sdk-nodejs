...
APIClient.prototype.resolvePrimaryPrivateKeyFromOptions = function(options, cb) {
    ...
    try {
        ...
        if (!options.primaryMnemonic && !options.primarySeed && !options.primaryPrivateKey) {
            throw new blocktrail.WalletInitError("Need to specify at least one of; Primary Mnemonic or Primary Seed or Primary PrivateKey");
        }
        ...
        if (options.primaryPrivateKey) {
            if (options.primaryPrivateKey instanceof bitcoin.HDNode) {
                ...
                options.primaryPrivateKey = bitcoin.HDNode.fromBase58(options.primaryPrivateKey, network);
                deferred.resolve(options);
            }
