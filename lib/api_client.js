...
var resolvePrimaryPrivateKey = function (primaryPrivateKey, primaryMnemonic, passphrase, network) {
    var deferred = q.defer();

    if (primaryPrivateKey && primaryPrivateKey instanceof bitcoin.HDNode) {
        deferred.resolve(primaryPrivateKey);
    } else if (primaryPrivateKey && !(primaryPrivateKey instanceof bitcoin.HDNode)) {
        deferred.resolve(bitcoin.HDNode.fromBase58(primaryPrivateKey, network));
    } else if (!primaryPrivateKey) {
        if (isNodeJS) {
            deferred.resolve(bitcoin.HDNode.fromSeedBuffer(bip39.mnemonicToSeed(primaryMnemonic, passphrase), network));
        } else {
            ...
            worker.postMessage({method: 'mnemonicToSeedHex', mnemonic: primaryMnemonic, passphrase: passphrase});
        }
    }
...
APIClient.prototype.initWallet = function (options, cb) {
    ...
    (function () {
        ...
        self.client.get("/wallet/" + identifier, null, true).then(function (result) {
            ...
            resolvePrimaryPrivateKey(primaryPrivateKey, primaryMnemonic, passphrase, network).then(function (primaryPrivateKey) {
