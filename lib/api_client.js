...
APIClient.prototype.resolvePrimaryPrivateKeyFromOptions = function (options, cb) {
    ...
    try {
        ...
        if (primaryPrivateKey) {
            ...
            self.mnemonicToPrivateKey(primaryMnemonic, passphrase).then(function (primaryPrivateKey) {
                return deferred.resolve([primaryMnemonic, primaryPrivateKey]);
            }, function (err) {
...
APIClient.prototype.resolveBackupPublicKeyFromOptions = function (options, cb) {
    ...
    try {
        ...
        if (backupPublicKey) {
            ...
            self.mnemonicToPrivateKey(backupMnemonic, "").then(function (backupPrivateKey) {
                backupPublicKey = backupPrivateKey.neutered();

                deferred.resolve([backupMnemonic, backupPublicKey]);
            }, function (err) {
                return deferred.reject(err);
            });
        }
...
APIClient.prototype.initWallet = function (options, cb) {
    ...
    (function () {
        ...
        self.client.get("/wallet/" + identifier, null, true).then(function (result) {
            ...
            var blocktrailPublicKeys = _.mapValues(result.blocktrail_public_keys, function(blocktrailPublicKey) { return bitcoin.HDNode.fromBase58(blocktrailPublicKey[0], self.network) });
            var primaryPublicKeys = _.mapValues(result.primary_public_keys, function(primaryPublicKey) { return bitcoin.HDNode.fromBase58(primaryPublicKey[0], self.network) });
            ...
            if (!readOnly) {
                wallet.unlock(options, function (err) {
                    if (err) {
                        return deferred.reject(err);
                    }
...
APIClient.prototype.createNewWallet = function (options, cb) {
    ...
    self.resolvePrimaryPrivateKeyFromOptions(options).spread(
        function (primaryMnemonic, primaryPrivateKey) {
            self.resolveBackupPublicKeyFromOptions(options).spread(
                        ...
                        function (result) {
                            ...
                            wallet.unlock(options, function (err) {
                                if (err) {
                                    ...
                                    deferred.resolve([wallet, primaryMnemonic, backupMnemonic, blocktrailPublicKeys]);
                                }
                            });
                        },
                        function (err) {
