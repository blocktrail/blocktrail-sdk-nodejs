...
var WalletSweeper = function(backupData, bitcoinDataClient, options) {
    ...
    switch (backupData.walletVersion) {
        ...
        case 3:
            // convert mnemonics to hex (bip39) and then base64 for decryption
            backupData.encryptedPrimaryMnemonic = Mnemonic.decode(backupData.encryptedPrimaryMnemonic);
            if (usePassword) {
                backupData.passwordEncryptedSecretMnemonic = Mnemonic.decode(backupData.passwordEncryptedSecretMnemonic);
            } else {
                backupData.encryptedRecoverySecretMnemonic = Mnemonic.decode(backupData.encryptedRecoverySecretMnemonic);
            }
